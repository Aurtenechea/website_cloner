...
# ══════════════════════════════════════════════════════════════════════════════
# LOG: escribe en consola Y en log.txt al mismo tiempo
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# 1. COOKIES (Lógica del primer script)
# ══════════════════════════════════════════════════════════════════════════════
...
def procesar_html_y_adjuntos(session: requests.Session, url: str, leccion_dir: Path):
    ...
    log(f"  [html crudo guardado] {raw_path.name} ({len(r.text)} chars)")
...
def descargar_video(url: str, curso_slug: str, nombre_video: str):
    ...
    log(f"  [video] iniciando yt-dlp → {nombre_video}.*")
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--cookies", str(COOKIES_FILE), # Se sigue pasando el txt local
            ],
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode == 0:
            ...
            log(f"  [error video] código {result.returncode}")
            log(f"  [yt-dlp stderr]\n{result.stderr[-1000:]}")
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. PROCESAMIENTO DE UNA LECCIÓN
# ══════════════════════════════════════════════════════════════════════════════
...
def procesar_leccion(session: requests.Session, url: str):
    ...
    descargar_video(url, curso_slug, nombre_video)
...
def main():
    ...
    # Cargamos directamente del archivo manual
